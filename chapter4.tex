...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}p{0.2\linewidth} p{0.4\linewidth} p{0.3\linewidth}@{}}
    ...
    Portability & Environmental independence for multi-device execution. & Achieved through Kotlin Multiplatform shared domain modules while preserving native platform interfaces. \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}p{0.25\linewidth} p{0.25\linewidth} p{0.45\linewidth}@{}}
    ...
    \midrule
    Mobile Frontend & Jetpack Compose (Android) & Prioritises a native Android UI for the project's primary deployment target, enabling direct profiling and memory tuning on constrained handsets while avoiding the overhead of a fully shared UI runtime~\cite{khan2019mobilephone,bendinelli2024composeflutter,wahlandt2024composexml}. \\
    Shared Mobile Logic & Kotlin Multiplatform & Reuses stable non-UI components such as API clients, domain models, validation rules, and cache policies across Android and iOS without sacrificing native user interfaces~\cite{skantz2023kmm}. \\
    Backend API & FastAPI (Python) & Selected for straightforward asynchronous endpoint development and tight integration with the Python-based machine learning workflow used by the forecasting module. \\
    Price Forecasting Model & CNN-LSTM & Deep learning architectures are well suited to volatile agricultural time series, while the local market dynamics identified in Pakistan further justify a robust forecasting pipeline for mandi price prediction~\cite{murugesan2022forecasting,habib2021price}. \\
    ...
    Database & PostgreSQL & Provides reliable relational storage for farmer profiles, mandi price histories, irrigation records, and financial entries while supporting structured querying across linked entities. \\
    ML Model Serving & TensorFlow Lite & Supports compact on-device inference for offline-capable components where model size, latency, and battery consumption must remain bounded on low-end smartphones. \\
    Language Support & Jetpack Compose localization + Urdu font resources & Supports an Urdu-first interface aligned with the accessibility needs of farmers who rely on mobile phones for agricultural information but may face literacy and usability barriers in text-heavy systems~\cite{khan2019mobilephone,fao2023dss}. \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[height=11cm, keepaspectratio]{Figures/4.1.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[height=11cm, keepaspectratio]{Figures/4.2.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[height=11cm, keepaspectratio]{Figures/4.3.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[height=11cm, keepaspectratio]{Figures/4.4.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[height=11cm, keepaspectratio]{Figures/4.5.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[height=11cm, keepaspectratio]{Figures/4.6.png}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}p{0.25\linewidth} p{0.15\linewidth} p{0.1\linewidth} p{0.1\linewidth} p{0.3\linewidth}@{}}
    ...
    Low-end device performance friction & Technical & Medium & Medium & Constrain Compose recomposition surfaces, rely on lazy rendering and local caching, and shift heavier ML inference workloads to backend tiers whenever latency budgets allow. \\
