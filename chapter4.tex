...
\begin{itemize}
    ...
    \item English language analytical interface
...
\begin{enumerate}
    \item The system \textbf{shall} allow the Policymaker to choose a primary crop (wheat, cotton, or rice) and generate a 30-day probabilistic mandi price forecast.
    ...
    \item The system \textbf{shall} trigger push notifications if an anomaly or significant 10\% short-term price jump is detected for the Policymaker's subscribed crop.
\end{enumerate}

\subsection{Crop Recommendation Module}
\begin{enumerate}
    \item The system \textbf{shall} accept explicit input parameters from the Policymaker including land size, district, and planting season.
...
\begin{enumerate}
    ...
    \item The system \textbf{shall} apply the designated crop coefficient ($K_c$) dynamically aligned with the Policymaker's active crop growth cycle to determine crop evapotranspiration ($ET_c$).
...
\begin{enumerate}
    ...
    \item The system \textbf{shall} process an expected yield input from the Policymaker combined with current mandi trading rates to formulate projected revenue.
    \item The system \textbf{shall} allow the Policymaker to record individual input overheads such as seed, fertilizer, and transport costs into an expense tracker.
...
\begin{enumerate}
    \item The system \textbf{shall} allow the Policymaker to register a distinct account utilizing their mobile number and standard OTP token confirmation.
    \item The system \textbf{shall} require the Policymaker to populate an initial profile detailing primary farm district and language interface preference.
    \item The system \textbf{shall} securely store established Policymaker profile data locally and synchronize passively to the cloud backend.
\end{enumerate}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}p{0.2\linewidth} p{0.4\linewidth} p{0.3\linewidth}@{}}
    ...
    Usability & Clarity and operational ease of the user interface. & Requires English, analytical charts, and non-complex structural design. \\
    Availability & Consistency of application and backend accessibility. & Necessitates offline resilience in rural Punjab. \\
    Scalability & Capability to expand user bases and data payloads. & Adapts to expanding PAMRA archives and Policymaker registrations. \\
    ...
    Portability & Environmental independence for multi-device execution. & Achieved utilizing modern native Android frameworks. \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{itemize}
    \item The UI \textbf{shall} integrate a fully functional professional English framework employing analytical data visualizations to accommodate highly literate policymakers.
...
\subsection{Security}
\begin{itemize}
    \item The system \textbf{shall} preserve stringent Policymaker data privacy policies, collecting zero arbitrary Personally Identifiable Information (PII) without transparent and explicit digital consent.
...
\begin{enumerate}
    \item The Policymaker possesses internet connectivity initially to accommodate application download and primary agricultural data synchronization.
    ...
    \item The Policymaker exhibits a high level of digital and analytical literacy required to manipulate touchscreen interfaces and interpret data.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{|p{0.3\linewidth}|p{0.65\linewidth}|}
    ...
    \hline
    \textbf{Actors} & Policymaker \\
    \hline
    \textbf{Summary} & Policymaker accesses a projected moving price trend for an agricultural commodity. \\
    \hline
...
\begin{table}[htbp]
  ...
  \begin{tabular}{|p{0.3\linewidth}|p{0.65\linewidth}|}
    ...
    \hline
    \textbf{Pre-Conditions} & Policymaker specifies identical active crop phase and planting baseline. \\
    \hline
...
\begin{table}[htbp]
  ...
  \begin{tabular}{|p{0.3\linewidth}|p{0.65\linewidth}|}
    ...
    \multicolumn{2}{|c|}{\textbf{Alternative Flow}} \\
    \hline
    \multicolumn{2}{|p{0.95\linewidth}|}{3a. If the Policymaker interacts with the Loan array, the module requests Principal, Interest rate constraints to formulate the cyclic installment value instantly.} \\
    \hline
  \end{tabular}
\end{table}
...
\begin{table}[htbp]
  \centering
  \caption{Use Case: Set Up Policymaker Profile}
  \label{uc:setup_profile}
  \begin{tabular}{|p{0.3\linewidth}|p{0.65\linewidth}|}
    \hline
    \textbf{Name} & Set Up Policymaker Profile \\
    \hline
    \textbf{Actors} & Policymaker \\
    \hline
...
\begin{table}[htbp]
  \centering
  \caption{Use Case: Switch Language to English}
  \label{uc:switch_urdu}
  \begin{tabular}{|p{0.3\linewidth}|p{0.65\linewidth}|}
    \hline
    \textbf{Name} & Switch Language to English \\
    \hline
    \textbf{Actors} & Policymaker \\
    \hline
    \textbf{Summary} & Transitions application terminology formats into standard English representations. \\
    \hline
    ...
    & 3. Restarts navigational layout matrix aligned LTR in English \\
    \hline
    \multicolumn{2}{|c|}{\textbf{Alternative Flow}} \\
    \hline
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}p{0.25\linewidth} p{0.25\linewidth} p{0.45\linewidth}@{}}
    ...
    \midrule
    Mobile Frontend & Jetpack Compose & Employs an efficient native Android foundation producing an artifact optimally structured for rural settings~\cite{khan2023cross}. \\
    ...
    Language Support & Android Resource Framework + Standard English typography & Crucial for delivering clear analytical insights to administrative policymakers~\cite{raza2021urduui}. \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[height=11cm, keepaspectratio]{"Figures/HomeDashboard (4.1).png"}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[height=11cm, keepaspectratio]{"Figures/Price Forecast (4.2).png"}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[height=11cm, keepaspectratio]{"Figures/Crop Recommendation (4.3).png"}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[height=11cm, keepaspectratio]{"Figures/Irrigation Schedule (4.4).png"}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[height=11cm, keepaspectratio]{"Figures/Financial Tools (4.5).png"}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[height=11cm, keepaspectratio]{"Figures/Settings (4.6).png"}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}p{0.2\linewidth} p{0.2\linewidth} p{0.25\linewidth} p{0.25\linewidth}@{}}
    ...
    \midrule
    policymaker\_id & UUID & PRIMARY KEY & Distinct internal account token \\
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}p{0.25\linewidth} p{0.15\linewidth} p{0.1\linewidth} p{0.1\linewidth} p{0.3\linewidth}@{}}
    ...
    Low-end device performance friction & Technical & Medium & Medium & Aggressively compile Jetpack Compose UI modules utilizing lightweight parameters and substitute heavy inferences onto server tiers where viable. \\
    Policymaker resistance to new platforms & Business & High & High & Maintain rigid conformity enforcing comprehensive analytical visual cues natively. \\
