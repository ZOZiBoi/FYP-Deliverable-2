...
\begin{itemize}
    ...
    \item \textbf{Hardware Constraints:} The application design explicitly caters to low-memory Android devices common among agricultural policymakers (i.e., devices with $<$ 2GB RAM), necessitating aggressive memory management.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Multilingual Interfaces:} The software constraints require dynamic LTR (Left-to-Right) rendering capabilities to fully support the standard English typography cleanly.
...
The diagram conceptually portrays the data transmission cycle. The Jetpack Compose application securely requests data parameters via RESTful principles directly to the FastAPI Backend. The server autonomously evaluates whether the request implies retrieving stable Postgres elements or necessitates real-time network interactions scaling to NASA POWER for evapotranspiration mechanics.
...
The structural representation explicitly models the relationships defining the software environment. It clarifies associative multiplicity existing between the primary entity \textit{Policymaker} and the underlying operational records.
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[
      ...
      \textbf{\hfil Policymaker \hfil}
      \nodepart{two}
      - policymaker\_id : UUID \\
      - name : String \\
      - phone : String \\
      - land\_size : Float
      \nodepart{three}
    ...
    % Relationships
    \draw[arrow] (policymaker.east) -- node[above, near start] {1} node[above, near end] {*} (crop.west);
    \draw[arrow] (crop.south) -- node[left, near start] {1} node[left, near end] {*} (price.north);
    \draw[arrow] (policymaker.south) -- node[right, near start] {1} node[right, near end] {*} (irrig.north);
