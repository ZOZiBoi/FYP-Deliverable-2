...
\newcommand{\fyptitle}{KisaanMadad: An Integrated Decision-Support System for Agricultural Policymakers in Punjab, Pakistan}
\newcommand{\degree}{BS}
...
% === ABSTRACT ===
\section*{Abstract}
Agricultural policymakers and macroeconomic planners in Pakistan face a confluence of structural challenges that severely constrain agricultural productivity and regional income: acute water scarcity, persistent crop price volatility, and the near-total absence of data-driven decision-support tools accessible at the administrative level. This report presents KisaanMadad (``Farmer Help''), an integrated decision-support system designed for agricultural policymakers and government agencies in Punjab, Pakistan. The system unifies four functionally distinct modules within a single native Android application: a probabilistic price forecasting engine employing a CNN-LSTM hybrid architecture benchmarked against Prophet and ARIMA baselines; a multi-criteria crop recommendation module that synthesises real-time weather, market proximity, planting calendars, and water availability; an irrigation scheduling engine grounded in the FAO-56 Penman-Monteith method, computing reference evapotranspiration $ET_0$ and applying crop-specific coefficients $K_c$ to derive regional water requirements; and a financial planning toolkit comprising macro loan calculators, revenue projections, and expense tracking. All modules draw on localised data sources --- Punjab mandi price records from PAMRA and AMIS alongside meteorological inputs from the NASA POWER API --- and are presented through an English-language analytical interface to serve highly literate policy stakeholders. Collectively, KisaanMadad is expected to reduce the information asymmetry experienced by administrators, improve regional irrigation efficiency, and provide actionable forecasts that enable more profitable and climate-resilient agricultural planning.
\pagebreak
% === END ABSTRACT ===
...
KisaanMadad is a native Android mobile application targeting wheat, cotton, and rice cultivation in Punjab, Pakistan. It consolidates four decision-support modules into a unified, English-language interface designed for agricultural policymakers and administrators. The Price Forecasting module delivers probabilistic commodity price predictions for major Punjab mandi markets. The Crop Recommendation module generates ranked planting suggestions by scoring candidate crops against current weather conditions, market access, seasonal calendars, and water availability. The Irrigation Scheduling module produces weekly regional irrigation schedules tailored to prevailing meteorological conditions. Finally, the Financial Planning module provides macro loan repayment calculators, crop revenue projections, and seasonal expense trackers --- functionality absent from all competing applications reviewed during this study.
...
KisaanMadad is expected to provide agricultural policymakers with actionable, localised intelligence that reduces systemic inefficiencies and mitigates the information asymmetry that currently characterises the mandi system. By embedding scientifically grounded irrigation scheduling directly on a mobile handset, the system aims to meaningfully improve regional water-use efficiency in a country facing acute water scarcity. Five principal contributions distinguish this work from the existing literature and commercial landscape: (1) it constitutes the first integrated platform to combine price forecasting, crop recommendation, irrigation scheduling, and financial planning within a single Pakistani agri-application; (2) it delivers FAO-56 compliant $ET_0$ computation on a mobile device for administrators planning macro water allocations; (3) it introduces financial planning tools demonstrably absent from all reviewed existing solutions; (4) all models are trained and evaluated on localised PAMRA/AMIS mandi data and NASA POWER meteorological feeds rather than global commodity indices; and (5) the application is built with an English-first interface, prioritising analytical depth for the literate policymaker population it serves.
\pagebreak
...
Although several agricultural technology solutions exist in the Pakistani market, they remain highly fragmented. Existing applications typically offer either weather forecasts or market prices in isolation, leaving a void for an integrated decision-support platform \cite{pasha2024agritech}. Most critically, none of the reviewed existing mobile solutions provide localised financial planning tools for macroeconomic planners \cite{pasha2024agritech}. To address these pressing systemic issues, KisaanMadad is proposed as an integrated decision-support system tailored for agricultural policymakers, aiming to bridge the information gap and promote sustainable, data-driven agricultural policies.

\section{Purpose of this Document}
The purpose of this document is to present the comprehensive design, implementation, and evaluation of KisaanMadad. In the face of fragmented existing solutions, KisaanMadad consolidates multiple disparate agricultural data streams into a unified, user-centric platform tailored for the specific needs of agricultural policymakers and administrators in Punjab, Pakistan. Thus, the primary research question addressed by this project is whether an integrated decision-support system can effectively deliver macro-level crop recommendations, precise regional irrigation planning, accurate market price forecasting, and crucial financial insights directly to government planners. To answer this, the document details the development of the system's four core modules: ML-based Price Forecasting, multi-criteria Crop Recommendation, FAO-56 compliant Irrigation Scheduling, and rule-based Financial Planning.
...
\begin{description}
    ...
    \item[Policymaker] An administrator or government official responsible for shaping agricultural strategies and overseeing regional resource allocation, typically possessing high digital and linguistic literacy in English.
\end{description}
...
\section{Conclusion}
To conclude, this chapter has provided a foundational understanding of the profound systemic challenges impeding Pakistan's agriculture sector, including severe water scarcity, widespread yield gaps, and significant market price volatility. The fragmented nature of current digital interventions necessitates the development of KisaanMadad, an integrated decision-support system aimed at empowering agricultural policymakers. The remaining document is structured to provide a comprehensive exploration of this proposed solution. Chapter 2 delves into an extensive literature review and analysis of related work. Chapter 3 discusses the system design and architecture of the proposed platform. Chapter 4 outlines the specific methodology employed for each of the four core modules. Chapter 5 details the technical implementation of the system. Chapter 6 presents the results and evaluation of the developed modules. Finally, Chapter 7 provides the conclusion and highlights avenues for future work.
% === END CHAPTER 1: INTRODUCTION ===
\chapter{Project Vision}

\section{Problem Domain Overview}
Agriculture constitutes 23.54\% of Pakistan's GDP and employs over a third of its workforce \cite{pakistan_economic_survey_2025}. However, an estimated 97\% of all farmers in Pakistan own and cultivate less than 12.5 acres of land \cite{rana2025farmers}, dominating the sector with smallholder operations that lack access to modern tools and data-driven insights. This productivity deficit results in severe yield gaps of 40-60\% \cite{adb2021pakistan}. Our integrated application centralises essential agricultural decision-making processes---from macro crop selection and regional irrigation planning to price forecasting and economic planning. By deploying these modules collectively, the system ensures more reliable, timely, and actionable intelligence for policymakers, ultimately enhancing regional profitability and broader national food security.

\section{Problem Statement}
The problem at hand involves the persistent reliance of Pakistani agricultural planners on fragmented, traditional heuristics rather than integrated, data-driven tools. This deficiency results in suboptimal agricultural policies across crop rotation, water utilisation, and market timing. By developing a unified, English-first decision-support system, we aim to automate complex predictive models and deliver actionable insights, thereby mitigating risks and improving overall macroeconomic efficiency.
...
Agricultural markets face extreme price volatility, and policymakers lack direct access to forward-looking, real-time wholesale mandi price forecasts. Existing applications fail to provide predictive insights using advanced machine learning models (e.g., CNN-LSTM) \cite{pide2022wheat}, leaving the agricultural economy vulnerable to systemic shocks \cite{habib2021price} and preventing proactive intervention.

\subsection{Irrigation Inefficiency}
Pakistan faces a severe hydrological crisis \cite{pcrwr2021water}, yet agricultural water use remains highly inefficient due to a reliance on traditional flood irrigation that causes massive volumetric waste \cite{worldbank2023water}. There is a distinct absence of accessible software tools that utilise scientifically validated evapotranspiration models, such as the FAO-56 standard \cite{allen1998crop}, to guide precise and efficient regional watering schedules for administrators.

\subsection{Fragmented Tools}
Current digital agricultural interventions are heavily siloed, offering discrete services like basic weather or isolated tips rather than a holistic platform \cite{pasha2024agritech}. Furthermore, these tools often lack the analytical depth and comprehensive English interfaces required by policy planners, making them largely ineffective for high-level macroeconomic planning \cite{fao2023dss}.

\subsection{Financial Exclusion}
The agriculture sector suffers from pervasive financial exclusion and a lack of formal planning capacity \cite{sbp2024finance}. The conspicuous absence of comprehensive financial modules in existing agri-tech applications heavily forces planners to make critical macroeconomic decisions based purely on intuition rather than mathematically projected revenues, tracking inputs, and managing systemic expenses.

\section{Goals and Objectives}
The goals and objectives of this project, addressing the lack of integrated decision-support for policymakers, are as follows:
\begin{enumerate}
    ...
    \item Deploy the system as an accessible, English-language analytical application for macroeconomic planners.
\end{enumerate}
...
The development phase focuses on creating the KisaanMadad platform, scoped geographically to Punjab, Pakistan, for wheat, cotton, and rice. The software encompasses four integrated modules: ML-based price forecasting, multi-criteria crop recommendation, FAO-56 compliant regional irrigation scheduling, and offline-capable macroeconomic financial calculators. The final deliverable is an English-supported analytical application. Real-time physical IoT sensors, computer vision disease detection, and direct micro-lending integrations are strictly excluded from this scope.
...
The first SDG is Zero Hunger. By actively providing agricultural policymakers with transparent data-driven crop recommendations and sophisticated forward-looking price forecasts, the integrated system directly addresses foundational knowledge gaps at the administrative level. This project supports the goal of doubling the agricultural productivity and baseline incomes of the agricultural sector \cite{un2030agenda}, promoting resilient practices that adapt to climate change through informed policy.
...
The second SDG we address is Industry, Innovation, and Infrastructure. Our solution leverages innovative computational models---like CNN-LSTM forecasts and FAO-56 calculations \cite{allen1998crop}---and translates them into a highly accessible analytical interface for planners. By doing so, we contribute to upgrading the fundamental technological capabilities of the agricultural sector's administrative bodies, improving data-driven governance without demanding prohibitive infrastructure investments \cite{un2030agenda}.

\section{Conclusion}
To conclude, this chapter establishes the foundational project vision by contextualising the profound challenges faced by agricultural planners in Pakistan, including market volatility, water scarcity, and fragmented tools. By clearly articulating an unambiguous problem statement, identifying highly specific objectives, and formally defining the project scope, the chapter demonstrates how an integrated decision-support system can simultaneously advance critical UN Sustainable Development Goals and empower policymakers. This sets the stage for a comprehensive exploration of our multifaceted approach in subsequent chapters.
...
% Adjust counter to ensure next chapter is Chapter 9 (since Chapter 8 is currently skipped)
